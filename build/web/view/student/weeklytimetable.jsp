                        ...
                        <td>
                            <c:forEach items="${requestScope.weeklyTimetable}" var="w">
                                <c:if test="${slot_index eq w.slot and w.date eq d}"> 
                                    <span style="color: #124DA3">${w.enrollment.group.course.course_code}</span> at ${w.classroom.room_code}</br>${w.professor.professor_code} -  
                                    <span style="${w.status eq 'present' ? 'color: green;' : w.status eq 'absent' ? 'color: red;' : 'color: gray;'}">
                                        ${w.status}
                                    </span>
                                    </br>
                                    (<fmt:formatDate value="${w.start_time}" pattern="HH:mm"/> - <fmt:formatDate value="${w.end_time}" pattern="HH:mm"/>)
                                    <a style="font-size : small;" href="ViewAttendance?semester_id=${w.enrollment.group.semester.semester_id}&group_id=${w.enrollment.group.group_id}&user_id=${sessionScope.user.user_id}">view</a>
                                </c:if>
                            </c:forEach>
                        </td>
                    </c:forEach>
